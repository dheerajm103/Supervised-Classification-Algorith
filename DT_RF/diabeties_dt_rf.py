...
df = pd.read_csv("Diabetes.csv")                        # importing dataset
df
...
plt.boxplot(df.iloc[:,0:8])                                # plotting boxplot for outliers
...
x = norm.iloc[:,0:8]                                       # predictors
y = norm.iloc[:,[8]]                                       # target
...
# plotting best tree
tree.plot_tree(cv_rf_clf_grid.estimators_[19], feature_names = x.columns,class_names=['0', "1"],filled=True)
